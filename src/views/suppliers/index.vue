    ...
    <Card noPadding>
      <template #header>
        <div class="flex items-center gap-3">
          <h1 class="text-xl font-bold text-slate-800">Lista de Fornecedores</h1>
          <span v-if="total > 0" class="bg-slate-200 text-slate-700 py-0.5 px-2.5 rounded-full text-sm font-medium">
            {{ total }}
          </span>
        </div>
        <div>
          <Button variant="primary" size="sm" @click="router.push('/suppliers/create')">
        ...
        <table class="w-full text-sm text-left text-slate-500">
          ...
          <tbody>
            ...
            <tr v-for="item in data" :key="item.id" class="bg-white border-b hover:bg-slate-50 transition-colors">
              ...
              <td class="px-6 py-4">{{ item.fantasy_name }}</td>
              <td class="px-6 py-4">{{ item.cnpj }}</td>
              <td class="px-6 py-4 text-right space-x-2">
                <ShowButton :to="`/suppliers/${item.id}`" />
                <EditButton :to="`/suppliers/${item.id}/edit`" />
              </td>
            </tr>
          </tbody>
        </table>
