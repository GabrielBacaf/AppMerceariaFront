...
const cancelSale = async () => {
  const result = await showAlert.confirm('Cancelar Edição', 'Tem certeza que deseja cancelar a edição?');
...
const handleSave = async () => {
  ...
  try {
    await apiService.update(route.params.id as string, payload);
    showAlert.success('Venda atualizada com sucesso!');
    router.push('/sales');
  } catch (error: any) {
    let errorMsg = 'Erro ao atualizar venda.';
...
const loadSale = async () => {
  try {
    ...
    deliveryPrice.value = data.delivery_price || 0;
    
    if (data.products) {
      cart.value = data.products.map((p: any) => ({
        product_id: p.id,
        barcode: p.barcode,
        name: p.name,
        quantity: p.pivot?.quantity || 1,
        unit_price: p.pivot?.unit_price || p.sale_value,
      }));
    }
    
    if (data.payments) {
      payments.value = data.payments.map((p: any) => ({
        value: p.value,
        payment_type: p.payment_type,
    ...
    showAlert.error('Erro ao carregar os dados da venda.');
  }
};
...
<template>
  <div class="min-h-[85vh] bg-slate-50 -m-6 p-4 md:p-6 flex flex-col font-sans">
    ...
    <!-- Top Header -->
    <div class="bg-gradient-to-r from-emerald-900 to-teal-900 text-white rounded-xl p-5 flex justify-between items-center shadow-lg mb-6">
      <div class="flex items-center space-x-4">
        <div class="bg-white/20 p-3 rounded-xl backdrop-blur-sm">
           <ShoppingCart class="w-8 h-8 text-white" />
        </div>
        <div>
          <h1 class="text-3xl font-extrabold tracking-tight">EDITAR VENDA #{{ route.params.id }}</h1>
          <p class="text-emerald-200 text-sm font-medium tracking-wide uppercase">Frente de Loja • PDV</p>
        </div>
      </div>
      <div class="text-right hidden md:block">
        <div class="text-sm text-emerald-200">Operador Caixa</div>
